...
def test_transcribe_with_context_doc_extraction_failure_is_silent(client):
    """_resolve_hotword_extraction (app.py, issue #406 dedup) feeds both the
    upload-time path here and POST /context. Upload time is a silent
    glossary side effect (see the docstring on _resolve_hotword_extraction
    and the try/except right after it in transcribe_audio): if extraction
    raises, the transcription itself must still complete with 200, never a
    500. This failure class had no coverage before.

    Mutation check: fails if the surrounding try/except in transcribe_audio
    is changed to re-raise instead of swallow (extraction failure would then
    surface as a 500 on the transcribe response, not a 200).
    """
    client.put("/api/providers/groq", json={"api_key": "fake-groq-key"})

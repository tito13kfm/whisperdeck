...
- Any diarization change (`gpt-4o-transcribe-diarize` rejected, see Background).
...
## Open risks (must be resolved during implementation, not guessed here)

1. **Multipart array encoding for `keywords`/`languages`.** OpenAI's docs describe the fields but not their raw multipart wire encoding (repeated key vs `keywords[]` vs JSON-encoded string in a single field). Must be confirmed against the real API (a live test call, or the current official Python/Node SDK source) before shipping, not assumed.
2. **`languages` replaces `language` for `gpt-transcribe`, and OpenAI's docs say not to send both.** `backends/openai.py`'s current code path always sends the singular `language` when set. The model-specific branch must omit `language` and send `languages` instead when keywords/multi-language context is used with `gpt-transcribe`.
3. **AssemblyAI `keyterms_prompt` phrase limit is 6 words per phrase**, unlike `keywords` which has no stated per-term length limit. Glossary terms are expected to be short (product names, acronyms) so unlikely to hit this, but not verified against real glossary content.
...
No code or tests are written as part of this spec/session. Per project testing tiers (AGENTS.md), when implemented this needs: unit coverage per touched backend (`tests/` already has per-backend test files for the existing providers to follow as a pattern) plus one live-flow check against a real API key for the `gpt-transcribe` keyword/language field-name risk above, since that can't be resolved from documentation alone.

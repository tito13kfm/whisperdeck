...
> **Status: MERGED to master via PR #72 on 2026-07-22** (merge commit 3d45fe9). Phase 0 through Phase 4 (Tasks 0-15) complete, plus a post-review hardening commit (0e92451) from a multi-agent review pass that fixed seven findings: relabel history invalidated on rediarize/queue-finalize resegmentation, ORM cascade from Transcript to RelabelHistory (SQLite FK cascade is inert), corrected_text after-image staleness check on undo, dual-mono rejection in diarize_live_stereo, stereo FLAC discarded on failed uploads, pyannote gating comment corrected, low-confidence predicate and newest-relabel query deduplicated. Full suite at merge: 326 passed, 2 deselected. Remaining known gaps (tracked in ROADMAP Known accepted gaps): (1) real-pyannote runtime verification for live-stereo diarization not done (dev machine has no pyannote/torch), and live-stereo coverage gaps can leave `speaker: None` lines at confidence 0.0 (bleed-dropped mic speech overlaps nothing); (2) "Undo relabel" button and confidence markers never driven in a real browser — unit-correct but runtime-unverified, and heuristic transcripts always score 1.0 so the markers can't fire without a pyannote-equipped machine. Phase 5 is contingent (only run if over-splitting persists after Phases 1-2 ship in production).

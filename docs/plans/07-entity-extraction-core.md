...
- **`topic` (added during part 3 design review, see docs/plans/09-topic-grouping.md) gets a third, distinct comparator: token-set Jaccard, not the char-level `SequenceMatcher` used for person/project.** Verified by direct measurement, not assumed: `SequenceMatcher` under-merges real topic paraphrases ("vendor renewal" vs "vendor contract renewal" scores 0.757, just under person/project's 0.82 threshold — reusing that threshold for topics would defeat the point of a "browsable topic index"). Token-set Jaccard (`len(set(a.split()) & set(b.split())) / len(set(a.split()) | set(b.split()))`, still stdlib, no new dependency) separates true paraphrases (0.5–0.667) from genuinely distinct topics (0.333–0.5) more cleanly, at a separate `topic_dedup_threshold` setting (default `0.6` — stated as provisional, not precisely tuned) rather than reusing `entity_dedup_threshold`, since it's a different metric on a different scale. **No separate token-count guard is needed at this threshold**: measured directly, a single-token topic name against a multi-word phrase containing it (`"migration"` vs `"billing migration"`, `"hiring"` vs `"hiring freeze"`) always scores exactly `0.5` against a 2-token superset and lower against longer ones — already below a `0.6` threshold, so the dangerous "generic single word swallows every specific topic containing it" failure mode is excluded by the threshold alone, with no extra mechanism required. **Named limitation, not silently accepted**: Jaccard penalizes length asymmetry, so a longer elaboration of a short topic (`"q3 budget"` vs `"q3 budget planning review"`, 2-vs-4 tokens) also scores exactly `0.5` — the same score as the attractor case above, but this time it's a real paraphrase that fails to merge. This is an accepted precision-over-recall tradeoff (falls back to a duplicate topic row, the same "just noise" cost part 1 already accepts for `decision`/`action_item`), not a solved problem — **if `topic_dedup_threshold` is ever tuned down toward 0.5, add an explicit "both names need ≥2 tokens, else require exact match" guard before doing so**, since below that point the threshold alone can no longer distinguish the attractor case from a genuine paraphrase (they score identically). `normalize_name`'s per-type length clamp needs a `topic` entry too (short phrase, similar to `TranscriptTag.tag`'s `VARCHAR(64)` precedent) — the current doc doesn't have one yet.
...
- `services/entities.py` (new): `normalize_name`, `fuzzy_ratio`, `find_best_fuzzy_match`, `jaccard_ratio` (topic comparator, added during part 3 review — see above), `plan_merge` (pure, no DB/LLM), `extract_entities` (LLM call + parse, never raises), `apply_extraction` (DB glue: load candidates, call `plan_merge`, persist), `delete_orphaned_entities`.
...
- `services/settings.py`: `DEFAULT_SETTINGS` gains `entity_dedup_threshold` (default `0.82`) and, per part 3, `topic_dedup_threshold` (default `0.6`, provisional).
- **Extraction prompt (added during part 3 review)**: the same single LLM call gains a fifth JSON key, `topics`, capped at `_MAX_TOPICS` (mirroring `services/tagging.py`'s existing `_MAX_TAGS = 5`), with phrasing deliberately matching `tagging.py`'s existing style guidance (short phrases, lowercase, no punctuation beyond hyphens) — this is what keeps exact-normalized-match viable as topic's fallback and limits duplicate-row proliferation. Per docs/plans/09-topic-grouping.md, this **replaces** `services/tagging.py`'s separate `"tagging"` job entirely rather than running alongside it — see that doc for the retirement plan.
...
class Entity(Base):
    ...
    type = Column(String(16), nullable=False)  # person | project | decision | action_item | topic
...
- [ ] `services/entities.py`: `normalize_name`, `fuzzy_ratio`, `find_best_fuzzy_match`, `jaccard_ratio`, `plan_merge` (three-way comparator dispatch: exact for decision/action_item, char-fuzzy for person/project, token-Jaccard for topic)
...
- [ ] Unit tests (topic, part 3): `jaccard_ratio` correctness on the measured pairs above (paraphrase pairs above threshold, distinct-topic pairs below, single-token-vs-superset pairs below threshold with no separate guard needed)

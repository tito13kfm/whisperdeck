...
if (typeof window !== 'undefined') {
  ...
  // Detail-poll test hooks for issue #435: exposes the schedule gate via
  // window.__testDetailPoll so tests can drive scheduleDetailPoll through
  // its real predicate (DETAIL_JOB_SLOTS + llmJobActive + S.page) without
  // re-implementing it. This does ship to all builds (robustness nit); a
  // future change could gate it behind a test-only flag if desired, but
  // the hooks are read-only closures over detail state and not a privilege
  // boundary — they only drive the existing poll predicate.

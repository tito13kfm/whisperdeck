...
- [x] A suite count, machine-dependent part separated out - the `1 skipped` above is a property of this machine, not of the branch, and the original box did not say so. Re-ran with `-rs` to name it: `SKIPPED [1] tests\test_voice_id.py:32: could not import 'torch': No module named 'torch'`, summary `934 passed, 1 skipped, 22 deselected, 1 warning in 267.59s`. `tests/test_voice_id.py:32` gates one test on `pytest.importorskip("torch")`, and this venv has no torch (`C:\Claude\WhisperDeck\.venv\Scripts\python.exe -c "import importlib.util as u; print(u.find_spec('torch') is not None)"` prints `False`). On a machine with torch installed the same command reports `935 passed, 0 skipped, 22 deselected`. Both are the same 957 collected tests and the same 935 selected; the only difference is whether that one torch-gated test runs. The reproducible, machine-independent claim is: 957 collected, 22 deselected by the `not e2e` marker, 935 selected, 0 failed, and every selected test that could run on the machine passed.
